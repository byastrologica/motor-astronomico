...
const BODY_DEFINITIONS = Object.freeze([
  ...
  ["NODO_NORTE_MEDIO", "Nodo Norte Medio", "NODE", true],
  ["NODO_NORTE_VERDADEIRO", "Nodo Norte Verdadeiro", "NODE", true],
...
const NODE_PAIRS = Object.freeze([
  ["NODO_NORTE_MEDIO", "NODO_SUL_MEDIO", "Nodo Sul Medio"],
  ["NODO_NORTE_VERDADEIRO", "NODO_SUL_VERDADEIRO", "Nodo Sul Verdadeiro"],
].map(([northId, southId, southName]) => ({ northId, southId, southName })));
...
export function generateRawChart(input) {
  ...
  for (const pair of NODE_PAIRS) {
    const northIndex = objects.findIndex((object) => object.id === pair.northId);
    if (northIndex >= 0) {
      objects.splice(
        northIndex + 1,
        0,
        createSouthNode(objects[northIndex], pair, context),
      );
    }
  }
